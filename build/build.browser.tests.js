...
const mochaVersion = pkgJson.devDependencies.mocha.replace(/[\^~*><=]/g, '')
const chaiVersion = pkgJson.devDependencies.chai.replace(/[\^~*><=]/g, '')
...
const testsJs = `
<script type="module">
  import * as ${camelise(pkgName)} from '${path.relative(templatePath, pkgJson.browser)}'
  window.${camelise(pkgName)} = ${camelise(pkgName)}
  import './tests.js'
  mocha.run()
</script>
`
...
fs.writeFileSync(dstFileName,
  template.replace(/{{TESTS}}/g, testsJs).replace(/{{PKG_NAME}}/g, pkgJson.name).replace(/{{MOCHA_VERSION}}/g, mochaVersion).replace(/{{CHAI_VERSION}}/g, chaiVersion)
)
